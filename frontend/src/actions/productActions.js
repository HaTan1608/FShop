...
export const listProducts = ({ name = '', category = '', gender = '', order = '', min = 0, max = 0, }) => async (dispatch) => {
    dispatch({
        type: PRODUCT_LIST_REQUEST
    });
    try {
        const { data } = await axios.get(
            `/api/products?name=${name}&category=${category}&gender=${gender}&min=${min}&max=${max}&order=${order}`
        );
        dispatch({
